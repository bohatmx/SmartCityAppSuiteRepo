...
public class BohaRequest extends Request<ResponseDTO> {
    ...
    public BohaRequest(int method, String url,
        ...
        Log.i(LOG, "...Server communication started ...");
    ...
    protected Response<ResponseDTO> parseNetworkResponse(
        ...
        Log.i(LOG, "parsing response, elapsed time since request start: "
                + getElapsed(start, System.currentTimeMillis()) + " seconds, size: " + response.data.length);
        try {
            Gson gson = new Gson();
            ...
            while ((entry = zis.getNextEntry()) != null) {
                ...
                Log.e(LOG, "## http response string length, unpacked: " + resp.length());
                dto = gson.fromJson(resp, ResponseDTO.class);
            }
        } catch (Exception e) {
    ...
    protected void deliverResponse(ResponseDTO response) {
        ...
        Log.i(LOG, "---- Server comms elapsed time: "
